...
- Name: auth/logout.spec.ts >> Logout Cart Behavior >> should convert authenticated cart to guest cart after logout
- Location: tests/e2e/auth/logout.spec.ts:259:3
...
```ts
  ...
  242 |     await expect(loginLink).toBeVisible();
  243 | 
  244 |     const userMenu = page.locator('[data-testid="user-menu-trigger"], [data-testid="profile-dropdown-trigger"]');
  245 |     await expect(userMenu).not.toBeVisible();
  246 |   });
  247 | });
  248 | 
  249 | test.describe('Logout Cart Behavior', () => {
  250 |   test.beforeEach(async ({ page }) => {
  251 |     await page.context().clearCookies();
  252 |     await page.goto('/');
